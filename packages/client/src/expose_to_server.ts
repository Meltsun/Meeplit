...
import { DynamicJSONRPCServer } from "./dynamic_json_rpc_server";
...
export function ExposeToServer<T extends Record<string, any>>(
    url: string,
    obj: T,
): void {
    const socket: Socket = io(url);

    const server = new DynamicJSONRPCServer(obj);
