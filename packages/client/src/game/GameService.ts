...
function makeInitialState() {
    ...
    const abilityImageUrl = ref<string>(resolveAssetUrl('/assets/未知卡牌.png').toString());
...
export default class GameService {
    ...
    public getSelectedCards(): Card[] {
        return this.state.selectedIndices.value
            .map((i) => this.state.handCards.value[i])
            .filter(Boolean);
    }
    ...
    public setPlayerInfo(info: PlayerInfo): void {
        ...
        this.state.abilityPlayerName.value = info.name || '';
    }
}
